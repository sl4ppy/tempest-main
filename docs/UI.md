...
This is not a single screen, but a continuous, looping sequence of states designed to attract players, display copyright and pricing information, show the high score table, and run a demonstration of the gameplay.
...
-   **Entry/Exit Conditions:**
    -   **Entry:** The Attract Mode is the default state of the game engine after the initial power-on and self-check routines are complete. It begins immediately.
    -   **Exit:** The loop is exited when a player inserts a coin and presses either the 1-Player or 2-Player Start button. This transitions the game state to `CNEWGA` (New Game).

### 1.2. Layout and Structure

-   **Dimensions and Coordinate System:** The game uses the Atari Vector Generator's coordinate system. All UI elements are vector-based drawings, positioned freely on the screen. There is no grid layout; positions are hardcoded in drawing routines.
...
-   **Atari "Fuji" Logo & Copyright:**
    ...
    -   **Asset Identifier:** The raw vector shape data is located in `ALVROM.MAC`. The text string is defined at label `EATARI` in `ALLANG.MAC`.
-   **Coin & Credit Information:**
    ...
    -   **Asset Identifier:** Pricing mode strings are at labels `ECMOD1-ECMOD3` and `ECMODE` in `ALLANG.MAC`. The "CREDITS" string is at `ECREDI`.
...
-   **Gameplay Demo:** The standard in-game HUD and playfield are displayed during the gameplay demonstration portion of the attract mode.

### 1.4. Text and Label Data

All on-screen text is stored as vector shape data. The primary source for these strings is `ALLANG.MAC`.
...
The "GAME OVER" and "PRESS START" messages are defined within the data sections of `ALSCOR.MAC`.

### 1.5. Colors and Visual Styling

-   **Color Palette:** Standard 16-color Atari vector palette. Colors are defined as constants in `ALCOMN.MAC` (e.g., `ZWHITE`, `ZRED`, `ZBLUE`).
...
-   **Color Usage:**
    ...
    -   Colors during the gameplay demo are handled by the main game rendering engine, with the playfield and enemies using their standard palettes.

### 1.6. Animation and Transitions

The Attract Mode is a state machine-driven sequence. The main game loop in `ALEXEC.MAC` checks the `QSTATE` variable each frame and jumps to the appropriate routine. The flow is as follows:
...
3.  **`CDLADR` (Display High Score Table):**
...
4.  **`CREQRAT` (Request Rating - Demo Setup):**
    -   **Action:** This is the setup phase for the demo game. The `PRORAT` routine in `ALWELG.MAC` is executed.
    ...
    -   It initializes the chosen level (`CONTOUR`), enemies (`INIENE`), and player (`INICUR`).
    -   It sets the game state `QSTATE` to `CNEWAV`.
5.  **`CNEWAV` / `NEWAV2` (New Wave Fly-In):**
    -   **Action:** The iconic "fly down the tube" animation plays.
    -   Upon completion, the state transitions to `CPLAY`.
6.  **`CPLAY` (Gameplay Demo):**
    -   **Action:** The main `PLAY` routine in `ALSCO2.MAC` runs. The game's AI now controls the player ship.
    -   The game plays itself until the player ship is destroyed.
7.  **`CENDLI` -> `CENDGA` (End Life / End Game):**
...
-   The "INSERT COIN" text flashes. This is controlled by the `D2GAME` routine in `ALSCOR.MAC`.
...
-   **Input:** The system listens for two primary inputs during the attract mode:
    1.  **Coin Insertion:** Monitored by the `COIN65.MAC` library. Inserting a coin increments the `$$CRDT` counter.
    2.  **Start Buttons:** The `MSTRT1` (1-Player) and `MSTRT2` (2-Player) inputs are checked every frame.
-   **Navigation Logic:** The `PROCRE` (Process Credits) routine in `ALEXEC.MAC` handles the logic. If `$$CRDT` is greater than zero and a start button is pressed, it will:
    ...
    4.  Change the `QSTATE` to `CNEWGA` to start a new game.
...
-   **`QSTATE` (`$00`):** A 1-byte variable holding the current main state code (e.g., `CLOGO`, `CPLAY`). This acts as an index into the `ROUTAD` function pointer table in `ALEXEC.MAC`.
-   **`QDSTATE` (`$01`):** A 1-byte variable holding the current display state code (e.g., `CDLOGP`, `CDHITB`). This tells the `INFO` routine in `ALSCO2.MAC` what informational text to draw.
...
**State Transition Example (from `ALEXEC.MAC`):**
```assembly
LOGINI:
...
The Attract Mode is the central hub of the game's non-interactive flow. It is a self-contained loop that demonstrates the game's features and waits for player interaction. Its only exit point is a transition to the "New Game" state, which hands control over to the player. Upon "Game Over," the game flow returns to this loop.
...
-   **Screen Identification and Purpose:**
    ...
    -   **When/How Shown:**
        1.  During the Attract Mode sequence, between the Atari logo and the gameplay demo.
        2.  After a game ends and the player *does not* achieve a new high score.
    -   **Entry/Exit Conditions:**
        -   **Entry:** `QSTATE` is set to `CDLADR` (Display Ladder) or `QDSTATE` is set to `CDHITB` (Display High Score Table).
        -   **Exit (Attract Mode):** A timer (`QTMPAUS`) expires, transitioning the game to the next state in the attract loop (`CREQRAT`).
        -   **Exit (Post-Game):** A timer expires, transitioning back to the beginning of the attract loop (`CLOGO`).
...
-   **Layout and Structure:**
    ...
    -   **Layout Logic:** The `LDRDSP` subroutine in `ALSCO2.MAC` is responsible for drawing this screen. It loops 8 times (defined by the `NHISCO` constant), calculating the vertical position for each new line of text.

-   **UI Elements and Data:**
    -   **Title Text:** "HIGH SCORES". String data is referenced by the `MHIGHS` label.
    ...
    -   **Fonts:** Standard game font, rendered by the `JSRL CHAR.` vector graphics routine.
...
-   **Screen Identification and Purpose:**
    ...
    -   **When/How Shown:** Shown immediately after a game ends if the `HISCHK` routine determines the player's score qualifies.
    -   **Entry/Exit Conditions:**
        -   **Entry:** The `HISCHK` routine successfully finds a place for the new score and sets `QSTATE` to `CGETINI` (Get Initials).
        -   **Exit:**
            1.  **Success:** The player presses the Fire button three times, locking in their initials. The game then transitions to display the updated high score table (`CDLADR`).
...
-   **Layout and Structure:**
    -   The layout is managed by the `GETDSP` routine in `ALSCO2.MAC`.
...
-   **State Management and Logic:**
    -   The entire process is managed by the `GETINI` state routine.
...
-   **Entry/Exit Conditions:**
    -   **Entry:** After a player presses Start and a new game begins, the state is set to `CINIRAT` (Initialize Rating).
    -   **Exit:**
...
-   The screen is drawn by the `RQRDSP` (Request Rate Display) routine in `ALSCO2.MAC`.
-   **Title:** "RATE YOURSELF"
-   **Instructional Text:** "SPIN KNOB TO CHANGE", "PRESS FIRE TO SELECT"
...
-   **Text Labels:**
    ...
    -   "NOVICE": `MNOVIC`
    -   "EXPERT": `MEXPER`
-   **Level Numbers:** The numbers are drawn procedurally using the standard character-drawing routines. The available levels are stored in the `LEVEL` table in `ALWELG.MAC`.
...
-   **State:** The main state is `CREQRAT` (Request Rate), which executes the `PRORAT` subroutine in `ALWELG.MAC`. The display state is `CDREQRA`.
...
-   **When/How Shown:** It is continuously displayed during active gameplay (`QSTATE` = `CPLAY`). It is hidden during the inter-level "warp" animation.
-   **Entry/Exit Conditions:** It appears when a level begins and disappears when the game ends or transitions to a different screen (like "Game Over").
...
-   **Layout Logic:** The layout is defined by a master template, `SCORES`, in `ALVROM.MAC`. At the start of a game, `INITEM` copies this template to a RAM buffer (`SCOBUF`). The `UPSCLI` routine then populates this buffer with the current game data. The X/Y positions are hardcoded into the template using `VCTR` commands.
...
-   **Life Icons:** A vector drawing of the player's ship. Up to 6 icons are displayed per player.
    -   **Asset Identifier:** The life icon shape is `LIFE0` in `ALVROM.MAC`. The routine displays one icon for each life remaining in the `LIVES1`/`LIVES2` variables.
-   **High Score:** The overall high score is displayed, fetched from the `HSCORL`... tables.
-   **Superzapper Status:** There is no persistent on-screen icon for the Superzapper. Its status is communicated through on-screen messages:
...
-   **Rendering Engine:** The `INFO` subroutine in `ALSCO2.MAC` is the master rendering routine for the HUD. It is called every frame when the game is in the `CDPLAY` display state.
...
-   **Screen Identification and Purpose:**
    ...
    -   **Entry/Exit Conditions:**
        -   **Entry:** `QSTATE` is set to `CENDGA`. The display state `QDSTATE` is set to `CDGOVR`.
        -   **Exit:** After a brief, timed pause, the game flow automatically proceeds to the high score check (`CHISCHK`). There is no player interaction.

-   **Layout and UI Elements:**
    -   **Text:** A large "GAME OVER" message is displayed in the center of the screen.
    -   **Asset Identifier:** The string is referenced by the `MGAMOV` label. The drawing logic is in the `DGOVER` subroutine in `ALSCO2.MAC`.
    -   The rest of the screen is typically black, with the playfield and HUD no longer being drawn.
...
-   **Screen Identification and Purpose:**
    ...
    -   **When/How Shown:** Occurs immediately after a wave is cleared. The `ENDWAV` routine sets `QSTATE` to `CNEWV2`.
    -   **Entry/Exit Conditions:**
        -   **Entry:** Game state is `CNEWV2`, which executes the `NEWAV2` routine in `ALWELG.MAC`.
        -   **Exit:** The animation is programmatic and time-based. Once the animation's target values are reached, `NEWAV2` automatically changes the `QSTATE` to `CPLAY` to begin the next level.
...
-   **Entry/Exit Conditions:**
    ...
    -   **Exit:** `MTEST` switch is deactivated. The game returns to the standard Attract Mode loop by setting the state to `CNEWGA`.

### 6.2. Layout and Structure

-   The diagnostic screen is a dense, multi-section display of text and data, all rendered by the `DSPSYS` routine in `ALTES2.MAC`. There is no single, consistent layout; it is a collection of different data readouts positioned in fixed locations.
-   **Sections Include:**
...
-   **Main State:** The test mode runs under the `CSYSTM` game state, executing the `SYSTEM` subroutine in `ALTES2.MAC`.
...
The Test Mode is a completely separate branch of the game's logic. It bypasses the entire player-facing game flow (Attract Mode, Gameplay, etc.). It has a single entry point (the physical test switch) and a single exit point (deactivating the switch), which cleanly resets the game and returns it to the Attract Mode. 